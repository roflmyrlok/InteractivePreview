...
{
    /// <summary>
    /// Helper class for extracting JWT claims consistently in ReviewService
    /// </summary>
    public static class JwtHelper
    {
        ...
        public static Guid GetUserIdFromToken(ClaimsPrincipal user, ILogger logger = null)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(userIdValue))
                {
                    ...
                    logger?.LogWarning("No user ID found in JWT claims. Available claims: {Claims}", allClaims);
                    throw new UnauthorizedAccessException($"Invalid or missing user identifier in the token. Available claims: {allClaims}");
                }

                if (!Guid.TryParse(userIdValue, out var userId))
                {
                    logger?.LogWarning("User ID value '{UserIdValue}' from claim '{ClaimType}' is not a valid GUID", userIdValue, foundClaimType);
                    throw new UnauthorizedAccessException($"User identifier '{userIdValue}' is not a valid GUID");
                }

                logger?.LogDebug("Successfully extracted user ID {UserId} from claim type '{ClaimType}'", userId, foundClaimType);
                return userId;
            }
        ...
        public static string GetUserRoleFromToken(ClaimsPrincipal user, ILogger logger = null)
        {
            if (user?.Identity?.IsAuthenticated != true)
            {
            ...
            try
            {
                ...
                foreach (var claimType in possibleRoleClaimTypes)
                {
                    ...
                    {
                        logger?.LogDebug("Found user role '{Role}' from claim type '{ClaimType}'", roleClaim.Value, claimType);
                ...
                {
                    logger?.LogDebug("Found user role '{Role}' from potential role claim '{ClaimType}'", potentialRoleClaim.Value, potentialRoleClaim.Type);
        ...
        public static string GetUsernameFromToken(ClaimsPrincipal user, ILogger logger = null)
        {
            if (user?.Identity?.IsAuthenticated != true)
            {
                logger?.LogWarning("User is not authenticated");
            ...
            try
            {
                ...
                foreach (var claimType in possibleUsernameClaimTypes)
                {
                    ...
                    {
                        logger?.LogDebug("Found username '{Username}' from claim type '{ClaimType}'", usernameClaim.Value, claimType);
